...
class MotorDataset(ImageDataset):
    def __init__(self, imdb_name, datadir, batch_size, im_processor,
                 processes=3, shuffle=True, dst_size=None):
        
        meta = imdb_name.split('_')
        self._db_name = meta[0]
        self._image_set = meta[1]
        
        super(MotorDataset, self).__init__(imdb_name, datadir, batch_size,
                                         im_processor, processes,
                                         shuffle, dst_size)
        self._devkit_path = os.path.join(datadir, '{0}_devkit'.format(self._db_name))
        self._data_path = os.path.join(self._devkit_path, self._db_name)
        assert os.path.exists(self._devkit_path), \
            'devkit path does not exist: {}'.format(self._devkit_path)
        assert os.path.exists(self._data_path), \
            'Path does not exist: {}'.format(self._data_path)

        self._classes = ("granule",)
    ...
    def evaluate_detections(self, all_boxes, output_dir=None):
        ...
        """
        self._write_track_results_file(all_boxes)
        self._do_python_eval(output_dir)
        if self.config['cleanup']:
            for cls in self._classes:
                if cls == '__background__':
                    continue
                filename = self._get_track_results_file_template().format(cls)
                os.remove(filename)
    ...
    def _write_track_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('Writing {} track results file'.format(cls))
            filename = self._get_track_results_file_template().format(cls)
